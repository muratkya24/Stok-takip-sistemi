...
// Context'leri ve hook'ları import ediyoruz
...
// ######################################################################
// # 1. BİLEŞEN: SATIŞ EKRANI (POS Terminali)
// ######################################################################
function SalesTerminal({ showModal }) {
    ...
    const handleBarcodeScan = (e) => {
        if (e.key === 'Enter') {
            ...
            axios.get(`https://muratkya244.pythonanywhere.com/api/products/by-barcode/${barcodeInput}`)
                ...
                .catch(err => {
                    showModal('Hata', `Bu barkoda sahip bir ürün bulunamadı: ${barcodeInput}`);
                });
        }
    };
    ...
    const handleCompleteSale = () => {
        if (basket.length === 0) {
            showModal('Uyarı', 'Satışı tamamlamak için önce sepete ürün eklemelisiniz.');
            return;
        }
        ...
        () => {
            ...
            axios.post('https://muratkya244.pythonanywhere.com/api/sales/bulk', saleData)
                ...
                .catch(err => {
                    showModal('Hata', `Satış tamamlanamadı: ${err.response?.data?.hata || 'Bilinmeyen bir hata oluştu.'}`);
                });
        });
    };
    ...
    return (
        <div className="pos-container">
            <div className="pos-basket-panel card">
                <h2 className="pos-title">Satış Sepeti</h2>
                <div className="basket-items">
                    {basket.length === 0 ? (<p className="info-text">Sepetiniz boş.</p>) : (
                        basket.map(item => (
                            <div key={item.id} className="basket-item">
                                <div className="item-info">
                                    <span className="item-name">{item.isim} ({item.renk}, {item.beden})</span>
                                    <span className="item-price">{item.quantity} x {parseFloat(item.satisfiyati).toFixed(2)} ₺</span>
                                </div>
                                <div className="item-actions">
                                    <button onClick={() => updateQuantity(item.id, -1)} className="quantity-btn"><MinusIcon /></button>
                                    <span className="item-quantity">{item.quantity}</span>
                                    <button onClick={() => updateQuantity(item.id, 1)} className="quantity-btn"><PlusIcon /></button>
                                    <button onClick={() => removeFromBasket(item.id)} className="delete-btn-small"><DeleteIcon /></button>
                                </div>
                            </div>
                        ))
                    )}
                ...
                <div className="basket-summary">
                    ...
                    <button onClick={handleCompleteSale} className="button primary-button complete-sale-btn" disabled={basket.length === 0}>Satışı Tamamla</button>
                </div>
            </div>
            <div className="pos-product-panel">
                <div className="card search-container">
                    <input type="text" className="form-input" placeholder="Barkodu Okutun veya Manuel Girin..." value={barcodeInput} onChange={(e) => setBarcodeInput(e.target.value)} onKeyDown={handleBarcodeScan} autoFocus />
                </div>
            </div>
        </div>
    );
...
// ######################################################################
// # 2. BİLEŞEN: STOK YÖNETİMİ SAYFASI
// ######################################################################
function ProductManagement() {
    ...
    return (
        <>
            <div className={`card form-container ${editingProductId ? 'edit-mode' : ''}`}>
                <h2 className="form-title">{editingProductId ? `Ürünü Düzenle (ID: ${editingProductId})` : 'Yeni Ürün Ekle'}</h2>
                <form onSubmit={handleSubmit} className="product-form">
                    <div className="form-field"><label htmlFor="isim">Ürün İsmi</label><input id="isim" name="isim" className="form-input" value={formData.isim} onChange={handleInputChange} placeholder="Örn: V Yaka Tişört" required /></div>
                    <div className="form-field"><label htmlFor="satisfiyati">Satış Fiyatı (₺)</label><input id="satisfiyati" name="satisfiyati" className="form-input" value={formData.satisfiyati} onChange={handleInputChange} type="number" step="0.01" placeholder="Örn: 299.90" required /></div>
                    <div className="form-field full-width"><label htmlFor="barcode">Barkod</label><div className="form-group"><input id="barcode" name="barcode" className="form-input" value={formData.barcode} onChange={handleInputChange} /><button type="button" onClick={handleGenerateBarcode} className="button secondary-button">Barkod Oluştur</button></div></div>
                    <div className="form-field"><label htmlFor="stokadedi">Stok Adedi</label><input id="stokadedi" name="stokadedi" className="form-input" value={formData.stokadedi} onChange={handleInputChange} type="number" placeholder="Örn: 50" /></div>
                    <div className="form-field"><label htmlFor="alisfiyati">Alış Fiyatı (₺)</label><input id="alisfiyati" name="alisfiyati" className="form-input" value={formData.alisfiyati} onChange={handleInputChange} type="number" step="0.01" placeholder="Örn: 150.00"/></div>
                    <div className="form-field"><label htmlFor="renk">Renk</label><input id="renk" name="renk" className="form-input" value={formData.renk} onChange={handleInputChange} placeholder="Örn: Siyah"/></div>
                    <div className="form-field"><label htmlFor="beden">Beden</label><input id="beden" name="beden" className="form-input" value={formData.beden} onChange={handleInputChange} placeholder="Örn: M" /></div>
                    <div className="form-field full-width"><label htmlFor="cinsiyet">Cinsiyet</label><select id="cinsiyet" name="cinsiyet" className="form-input" value={formData.cinsiyet} onChange={handleInputChange}><option value="">Cinsiyet Seçin...</option><option value="Kadın">Kadın</option><option value="Erkek">Erkek</option><option value="Unisex">Unisex</option></select></div>
                    <div className="form-buttons full-width">{!editingProductId && <button type="button" onClick={handleClearForm} className="button secondary-button">Formu Temizle</button>}{editingProductId && <button type="button" onClick={handleCancelEdit} className="button secondary-button">İptal</button>}<button type="submit" className="button primary-button">{editingProductId ? 'Değişiklikleri Kaydet' : 'Yeni Ürün Ekle'}</button></div>
                </form>
            </div>
            <div className="card product-table-container">
                <div className="table-header"><h2 className="table-title">Stok Listesi (Gruplanmış)</h2><input type="text" className="form-input table-search-input" placeholder="Ürün Adı veya Barkod Ara..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/></div>
                {loading ? <p className="info-text">Yükleniyor...</p> : (
                    <table className="product-table grouped-table">
                        <thead><tr><th>Ürün İsmi</th><th>Cinsiyet</th><th>Renk</th><th>Beden</th><th>Barkod</th><th>Stok</th><th>Alış Fiyatı</th><th>Satış Fiyatı</th><th className="actions-cell">İşlemler</th></tr></thead>
                        <tbody>{filteredGroupedProducts.map(group => (<GroupedProductRow key={group.urun_ismi} productGroup={group} selection={selections[group.urun_ismi]} onSelectionChange={handleSelectionChange} onEdit={handleEditClick} onDelete={handleDelete}/>))}</tbody>
                    </table>
                )}
            </div>
        </>
...
// ######################################################################
// # 3. BİLEŞEN: SATIŞ RAPORLARI SAYFASI
// ######################################################################
function SalesReport({ showModal }) {
    ...
    const openReturnModal = (sale) => {
        setReturnModal({ isOpen: true, sale: sale, quantity: sale.adet });
    };

    const closeReturnModal = () => {
        setReturnModal({ isOpen: false, sale: null, quantity: 1 });
    };

    const confirmReturn = () => {
        const quantity = parseInt(returnModal.quantity, 10);

        if (isNaN(quantity) || quantity < 1 || quantity > returnModal.sale.adet) {
            showModal('Geçersiz Adet', `Lütfen iade edilecek adet için 1 ile ${returnModal.sale.adet} arasında bir sayı girin.`);
            return;
        }

        handleReturn(returnModal.sale.id, quantity, () => {
            closeReturnModal();
            setDetailsModal({ isOpen: false, transaction: null }); // Detay modalını da kapat
        });
    };

    return (
        <div className="sales-report-container">
            <h1 className="page-title">Satış Raporları</h1>
            <div className="card report-controls">
                <div className="date-filter-container"><label htmlFor="sales-date">Rapor Tarihi:</label><input type="date" id="sales-date" className="form-input" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} /></div>
                <div className="report-summary">
                    <div className="summary-card"><h3>Toplam İşlem</h3><span>{groupedSales.length} adet</span></div>
                    <div className="summary-card"><h3>Günlük Ciro</h3><span>{totalRevenue.toFixed(2)} ₺</span></div>
                    <div className="summary-card profit"><h3>Günlük Kâr</h3><span>{totalProfit.toFixed(2)} ₺</span></div>
                </div>
            </div>
            <div className="card product-table-container">
                {loading ? <p className="info-text">Rapor Yükleniyor...</p> : (
                    <table className="product-table">
                        <thead>
                            <tr><th>İşlem Zamanı</th><th>Ürün Sayısı</th><th>Toplam Tutar</th><th className="actions-cell">Detaylar</th></tr>
                        </thead>
                        <tbody>
                            {groupedSales.length > 0 ? groupedSales.map(transaction => (
                                <tr key={transaction.id}>
                                    <td>{new Date(transaction.timestamp).toLocaleString('tr-TR')}</td>
                                    <td>{transaction.totalItems} adet</td>
                                    <td>{transaction.totalAmount.toFixed(2)} ₺</td>
                                    <td className="actions-cell">
                                        <button onClick={() => setDetailsModal({ isOpen: true, transaction: transaction.items })} className="button secondary-button" style={{padding: '0.5rem 1rem'}}>Detayları Gör</button>
                                    </td>
                                </tr>
                            )) : (<tr><td colSpan="4" className="info-text" style={{padding: '2rem'}}>Seçilen tarihte satış bulunmamaktadır.</td></tr>)}
                        </tbody>
                    </table>
                )}
            </div>
            ...
            {detailsModal.isOpen && (
                <Modal isOpen={detailsModal.isOpen} onClose={() => setDetailsModal({ isOpen: false, transaction: null })} title={`İşlem Detayları (${new Date(detailsModal.transaction[0].satis_tarihi).toLocaleString('tr-TR')})`} size="lg">
                    <table className="product-table">
                        <thead><tr><th>Ürün İsmi</th><th>Barkod</th><th>Adet</th><th>Birim Fiyat</th><th>Toplam</th><th className="actions-cell">İade</th></tr></thead>
                        <tbody>
                            {detailsModal.transaction.map(sale => (
                                <tr key={sale.id}>
                                    <td>{`${sale.urun_ismi} (${sale.cinsiyet || 'N/A'}, ${sale.renk || 'N/A'}, ${sale.beden || 'N/A'})`}</td>
                                    <td>{sale.barcode}</td>
                                    <td>{sale.adet}</td>
                                    <td>{parseFloat(sale.satis_fiyati || 0).toFixed(2)} ₺</td>
                                    <td>{(parseFloat(sale.satis_fiyati || 0) * sale.adet).toFixed(2)} ₺</td>
                                    <td className="actions-cell">
                                        <button onClick={() => openReturnModal(sale)} className="icon-button return-button" title="Satışı İade Al"><ReturnIcon /></button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
            ...
            {/* Kısmi İade Modalı */}
            {returnModal.isOpen && (
                <Modal isOpen={returnModal.isOpen} onClose={closeReturnModal} onConfirm={confirmReturn} title={`'${returnModal.sale.urun_ismi}' İade İşlemi`}>
                    <div>
                        <p>Bu üründen kaç adet iade etmek istiyorsunuz?</p>
                        <input
                            type="number"
                            className="form-input"
                            value={returnModal.quantity}
                            onChange={(e) => {
                                setReturnModal(prev => ({ ...prev, quantity: e.target.value }));
                            }}
                            min="1"
                            max={returnModal.sale.adet}
                            style={{ marginTop: '1rem', width: '100px' }}
                            autoFocus
                        />
                        <p style={{ fontSize: '0.8rem', color: '#64748b', marginTop: '0.5rem' }}>
                            (Maksimum iade adedi: {returnModal.sale.adet})
                        </p>
                    </div>
                </Modal>
            )}
